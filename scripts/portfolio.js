document.addEventListener('DOMContentLoaded', () => {
  ...
  if (submitBtn) {
    submitBtn.addEventListener('click', async () => {
      ...
      try {
        ...
        showMessage('error', '⚠ Sending failed. Please email me directly at er.aamirsohail@proton.me');
      } finally {
        setLoading(false);
      }
    });
  ...
  // ── RESUME PDF URL ──
  // Replace this with your actual hosted PDF URL.
  // Options: Google Drive share link (use /preview suffix),
  //          Dropbox link (change ?dl=0 to ?raw=1),
  //          or any direct .pdf URL.
  const RESUME_PDF_URL = './resume/Aamir_Resume.pdf'; // e.g. 'https://drive.google.com/file/d/YOUR_FILE_ID/preview'

  const resumeModal    = document.getElementById('resume-modal');
  const resumeBtn      = document.getElementById('resume-btn');
  const resumeCloseBtn = document.getElementById('resume-close-btn');
  const resumeDlBtn    = document.getElementById('resume-dl-btn');
  const resumeFrame    = document.getElementById('resume-frame-wrap');

  function openResume() {
    resumeFrame.innerHTML = '';
    if (RESUME_PDF_URL) {
      resumeDlBtn.href = RESUME_PDF_URL;
      const iframe = document.createElement('iframe');
      iframe.src = RESUME_PDF_URL;
      iframe.title = 'Aamir Sohail Resume';
      resumeFrame.appendChild(iframe);
    } else {
      resumeDlBtn.href = '#';
      resumeFrame.innerHTML = `
        <div class="resume-no-pdf">
          <svg width="48" height="48" viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.2" stroke-linecap="round"><rect x="3" y="1" width="10" height="14" rx="1.5"/><path d="M6 5h4M6 8h4M6 11h2"/></svg>
          <div><strong>No resume uploaded yet</strong>
          Set your PDF URL in the <code>RESUME_PDF_URL</code> variable inside the script.</div>
        </div>`;
    }
  ...
  function closeResume() {
    ...
    document.body.style.overflow = '';
    setTimeout(() => { resumeFrame.innerHTML = ''; }, 300);
  }
